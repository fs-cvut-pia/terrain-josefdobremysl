...
bool Lod::find() {
	...
	while (Body.size() > 1 || n < 50) {
		...
		for (int i = -1; i < 2; i++) {				// Cyklus přes všechny sousedy aktuálního bodu 
			for (int j = -1; j < 2; j++) {
				...
				Point testovanyBod(k_i, l_j);
				
				if ((k_i != start.x || l_j != start.y) && map.alt(testovanyBod) <= 0) {       // testování bodu
	...
	string filename = "C:\\Users\\josef\\Desktop\\MAPA_lod.dat";   // Otevření souboru pro zápis
	...
	while (true) {
		...
		if (aktualni_bod2 == start) {
			string filename = "C:\\Users\\josef\\Desktop\\Lod.dat";
